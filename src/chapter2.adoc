...
The WARL XLEN-wide HAC Runtime CSR, `hacr` provides an interface for software
to interpret and manage the SCC allocations in the register file.
...
The `hacr` register contains a slot, `hacmulN` and `hacmulN_valid`, for each
...
is to reduce the complexity of the software that manages the `hacr` and
...
On reset, the `hacr` register contains zeroes.
